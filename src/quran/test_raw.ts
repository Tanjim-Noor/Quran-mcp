...
async function runTests() {
  ...
  const envType = process.env.QURAN_ENV?.toLowerCase() === "pre-production" ? QuranEnvironment.PRE_PRODUCTION : QuranEnvironment.PRODUCTION;
  ...
  // Test 3: Fetch verse with English translation and Tafsir
  ...
  try {
    ...
    console.error("❌ Failed to fetch verse with translation and tafsir");
    console.error(`   Error: ${error.message}`);
    console.log("");
  }
